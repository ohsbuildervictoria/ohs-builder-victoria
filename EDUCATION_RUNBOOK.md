...
**Public gateway (22 Aug 2026, commit `ec7fe21`, merge `01a5caf`):** `/education` is now the public Education landing page (`src/pages/EducationLanding.jsx`); `/go` remains the role router. Portal buttons link to `/login?portal=institution|assessor|student` — the same sign-in with Education framing only (`src/pages/Login.jsx`), then `/go`. Invited people keep using `/edu/join/:token`. Homepage + Pricing nav gained "OHS Builder Education" (compact mark + "Education" label below `sm`). `POST /api/education-enquiry` (`functions/api/education-enquiry.js`) emails the support inbox only — fixed recipient, no auto-reply, same-origin check, honeypot, 3 s minimum fill time, 8 KB cap, best-effort per-IP limit via the Cache API; nothing is provisioned (create institutions from `/platform`). No migration, no RLS/RPC change.
